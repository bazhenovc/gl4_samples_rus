...
void main()
{
	if ( I == 0 )
	{
		...
		gl_TessLevelOuter[2] = 1.0;	//Level( Input[1], Input[2] ) * k;
		gl_TessLevelOuter[0] = 1.0;	//Level( Input[0], Input[2] ) * k;
		gl_TessLevelOuter[1] = 1.0;	//Level( Input[0], Input[1] ) * k;
		gl_TessLevelInner[0] = unMaxTessLevel;/*max( max( gl_TessLevelOuter[0], gl_TessLevelOuter[1] ),
...
void main()
{
	//vec3	norm	= texture( unNormalMap, Input.vTexcoord ).rgb;
